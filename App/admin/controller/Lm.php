...
class Lm extends Common
{
	...
	public function index()
	{
		...
		addlog('Lm/index','栏目列表',Db::name('lm')->getLastSql());		
		return view();
	}
...
public function sanji($topid)
{
	...
	foreach($TypeList as $k=>$v)
	{
		...
		*/
		...
		/////
			...
			<a id="color'.$v[Depth].'" style="'.$strbgclass.'" href="'.url("Lm/updata",array('id'=>$v['id'])).'">'.$v[TypeName].'   <font style="color:#666; font-size:10px; font-weight:normal;"> [ID:'.$v[id].']'.$v[id1path].'</font>'.$is_meau.'<font style="color:#666; font-size:10px; font-weight:normal;">(文档:'.$newsnum.')</font></a>
			</td>
			<td align="right">
			'.$tjzl.'
			</td>
    ...
    public function add(){
		...
		if(request()->isPost())
		{
				...
				$data['temp'] = trim($POST['temp']);
				$data['ljdz'] = trim($POST['ljdz']);
				if($data['temp'] == '') $data['temp'] = 'about';
				$data['contemp'] = trim($POST['contemp']);
			...
			if($num>0)
			{
				...
				addlog('Lm/add',$data['TypeName'].'/新增栏目成功',Db::name('lm')->getLastSql());		
				msg('1','数据已添加');exit();
			}else{
				addlog('Lm/add',$data['TypeName'].'/新增栏目失败',Db::name('lm')->getLastSql());		
				msg('0','数据未添加');exit();
			}
		}
		...
		if($num>0)
		{
		   //更新当前栏目下所有栏目的idpath等参数
			...
			foreach($oldlist as $oldk=>$oldv)
			{
				...
				$num3=Db::name('lm')->where("`id`='".$oldv['id']."'")->update($data1);
				if($num3<1){
					//addlog('Lm/updata_oldv',$data['TypeName'].'/修改栏目成功,更新下级栏目idpath失败',Db::name('lm')->getLastSql());			
				}
			}
			...
			if($_POST['jcmb']=='1')
			{
				...
				foreach($likelist as $k1 => $v1)
				{
					...
					//exit;
					if($num1<1){
				    // addlog('Lm/updata',$data['TypeName'].'/修改栏目成功,子栏目模板更换失败',Db::name('lm')->getLastSql());			
			...
			addlog('Lm/updata',$data['TypeName'].'/修改栏目成功',Db::name('lm')->getLastSql());	
			msg('1','数据已修改',url('Lm/index'));exit();
			//parent::getnews("添加成功","run.php?m=page&a=pagetype");
		}else{
			addlog('Lm/updata',$data['TypeName'].'/修改栏目失败',Db::name('lm')->getLastSql());	
			msg('0','数据未修改');exit();
		}
	}
