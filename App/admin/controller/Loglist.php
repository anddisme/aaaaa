...
class Loglist extends Common
{
	...
	public function index()
	{
		...
		$kw=get('kw');
        if(!empty($kw)){ $and=" and (`UserName` like '%".$kw."%' || `endtime` like '%".$kw."%' || `model` like '%".$kw."%' || `content` like '%".$kw."%' || `ip` like '%".$kw."%' )"; }
		
		$list=db::name('log')->where("`id`>'0' and `is_del`<>'1' and `model`<>'Log/index' $and")->order('`id` desc')->paginate($PageSize);
		///echo db::name('log')->getLastSql();
		///exit;
		...
		addlog('Log/index','获取日志列表',Db::name('log')->getLastSql());
		return view();
	}
