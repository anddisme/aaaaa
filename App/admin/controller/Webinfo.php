...
class Webinfo extends Common
{
    ...
    public function add(){
        ...
        if(request()->isPost())
        {
			...
			//print_r($filed);die;
			if($filed['id'] >0){
				addlog('Webinfo/add',$name.'/新增网站配置字段重名，请重新输入字段名',Db::name('webinfo')->getLastSql());
            ...
            if($num>0)
            {
				addlog('Webinfo/add',$name.'/新增网站配置字段成功',Db::name('webinfo')->getLastSql());
                echo json_encode(array("status"=>'1',"message"=>"更新成功"));
                exit();
            }else{
				addlog('Webinfo/add',$name.'/新增网站配置字段失败',Db::name('webinfo')->getLastSql());
    ...
    public function index(){
        ...
        if(request()->isPost())
        {
			...
			for ($x=0; $x<=$keyscoun-1; $x++) {
			    ...
			    addlog('Webinfo/index','更新网站配置成功',Db::name('webinfo')->getLastSql());
				echo json_encode(array("status"=>'1',"message"=>"更新成功"));
				exit();			

	}

			$webinfo=db::name("webinfo")->order("`id` asc")->select();
	...
	public function delid()
	{
		...
		$id = $_REQUEST['id'];
		if($id>0){
			$num = Db::name("webinfo")->where("`id` = $id")->delete();
			//return 1;
//			return Db::name("webinfo")->getLastSql();
			if($num>0){
				addlog('Webinfo/delid','删除网站配置成功',Db::name('webinfo')->getLastSql());
				return 1;
			}else{
				addlog('Webinfo/delid','删除网站配置失败',Db::name('webinfo')->getLastSql());
				return 0;
			}
		}
