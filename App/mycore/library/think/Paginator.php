...
abstract class Paginator
{
     ...
     * @return string
     */
    protected function url($page,$type)
    {
        if ($page <= 0) {
            $page = 1;
        }
		...
		//return var_export($pathinfo);
		$url='';
		/*
		if(!empty($query) && $query!='/')
		{
			//return '===';
			 $url=$pathinfo['path'].'?'.$query.'&page='.$page;
		}else{
			 $url=$pathinfo['path'].'?page='.$page;
		}
		*/
		
		//$url=str_replace('[PAGE]', $page, $path);
		//$url=preg_replace("page=/[0-9]/",$page,$path);
		$urlpath=$pathinfo['path'];
		$ispage=strpos($_SERVER['REQUEST_URI'],'?');
		if($ispage)
		{
			$list=$_GET;
			unset($list['page']);
			$url0=http_build_query($list);
			$url='?'.$url0.'&page='.$page;
		}else{
			 $url='?page='.$page;
	    }
