...
Popup.prototype = {
    ...
    build: function() {
        ...
        var l = '<div id="dialogBoxClose" style="width:58px; color:red; height:18px;cursor:pointer;margin-bottom:2px;">关闭<div>';
        ...
        var f = '<div id="dialogBox" style="border:1px solid #a78139;display:none;z-index:' + k + ";position:relative;width:" + this.config.width + 'px;background:url(\'/images/bgs1.gif\') repeat-x scroll 0 -68px #a78139;padding:1px;"><table width="100%" border="0" cellpadding="0" cellspacing="0" style="background:url(\'/images//bgs1.gif\') repeat-x 0 -144px">';
        if (this.config.isHaveTitle) {
            f += '<tr height="29"><td style="background:url(\'/images//edithelper1.gif\') no-repeat right -68px transparent;"><table style="-moz-user-select:none;height:24px;" width="100%" border="0" cellpadding="0" cellspacing="0" ><tr><td width="6" height="24"></td><td id="dialogBoxTitle" style="color:' + this.color.wColor + ';font-size:14px;font-weight:bold;">' + this.info.title + '&nbsp;</td><td id="dialogClose" width="18" align="right" valign="middle">' + l + '</td><td width="9"></td></tr></table></td></tr>'
        } else {
            f += '<tr height="10"><td align="right">' + l + "</td></tr>"
        }
        f += '<tr id="container-tr" style="height:' + (this.config.height - 36) + 'px" valign="top"><td style="position:relative;padding:1px;"><div style="border:1px solid #FFF;"><div style="background:url(\'/images//bgs1.gif\') repeat-x #FFF;height:13px;line-height:13px;font-size:1px;">&nbsp;</div><div id="dialogBody" style="background-color:#FFF;height:' + (this.config.height - 46) + "px;width:" + (this.config.width - 4) + 'px"></div></div></td></tr></table></div><div id="dialogBoxShadow" style="display:none;z-index:' + b + ';"></div>';
...
function relogin1(url,title,width,height) {
    g_pop_a = new Popup({
    ...
    g_pop_a.setContent("contentUrl", url);// 设置窗口内容
    g_pop_a.build();
    g_pop_a.show();
}
...
function relogin2() {
    ...
    g_pop.setContent("title", "弹出窗口html");//设置窗口标题名称
    g_pop.setContent("contentHtml", "内容<br/>.......<br/>内容");// 设置窗口内容
    g_pop.build();
    g_pop.show();
}
...
function relogin3() {
    ...
    g_pop.setContent("title", "弹出窗口confirm");//设置窗口标题名称
    g_pop.setContent("confirmCon", "内容<br/>.......<br/>内容");// 设置窗口内容
...
function relogin4() {
    ...
    g_pop.setContent("title", "弹出窗口alert");//设置窗口标题名称
    g_pop.setContent("alertCon", "内容<br/>.......<br/>内容");// 设置窗口内容
    g_pop.build();
    g_pop.show();
}
